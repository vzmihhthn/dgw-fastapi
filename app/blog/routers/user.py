...
def create_all_users(request: List[schemas.User], db: Session = Depends(database.get_db), current_user: schemas.User = Depends(oauth2.get_current_user)):
    return user.create_all_users(request, db)


@router.get("/", response_model=list[schemas.ShowUser], status_code=200 ) #get all users
def get_users(request: schemas.User,db: Session = Depends(database.get_db),current_user: schemas.User = Depends(oauth2.get_current_user)):
